...
//	}
	...
	private String createParseInputMessage(String key){
		...
		case "idade":
			...
			break;
		case "cnpj":
			value = JOptionPane.showInputDialog("Insira CNPJ:");
			if (validCNPJ(value)){
				return value;
			}
		case "pgto":
			value = JOptionPane.showInputDialog("Insira pagamento:");
	...
	public boolean validCNPJ (String i) {
		try{
			String parseValue = i.replaceAll("\\/", "").replaceAll("\\.", "");
			return (parseValue.length() == 14) ? true : false;
		}catch(Exception e){
			e.printStackTrace();
			return false;
		}
